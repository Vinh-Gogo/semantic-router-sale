...
def predict_3_class(input_text: str):
    if not input_text or not input_text.strip() or classifier is None:
        return "BINH_THUONG", {}

    # 🔥 FIX 1: Chuẩn hóa y hệt lúc Train
    # Giả sử anh đã có hàm normalize_vnese, hãy gọi nó. Nếu chưa, ít nhất phải .lower()
    clean_text = input_text.lower().strip()
    # clean_text = normalize_vnese(clean_text) # Gọi nếu anh có hàm này
    
    # 2. Nhúng câu text đã làm sạch
    input_vec = np.array(embeddings.embed_query(clean_text)).reshape(1, -1)
    
    probabilities = classifier.predict_proba(input_vec)[0]
    scores = {label: round(prob * 100, 2) for label, prob in zip(classifier.classes_, probabilities)}
    
    max_prob = max(probabilities)
    predicted_label = classifier.classes_[np.argmax(probabilities)]
    
    # Nếu AI không đủ tự tin (60%), trả về BINH_THUONG để an toàn
    if max_prob < CONFIDENCE_THRESHOLD:
        return "BINH_THUONG", scores
